...
{
    NSString *regex = @"(.*) EXECUTION_DATA: .* (\\w{3}.\\w{3}) .* orderId:(\\d+): clientId:(\\w+), execId:(.*.01), .*, acctNumber:(\\w+).*, side:(\\w+), shares:(\\d+), price:(.*), permId:(\\d+).*";
    XCTAssertTrue([self.candidate isMatchedByRegex:regex range:NSMakeRange(0, [self.candidate length])], @"Match has failed!");
...
- (void)testIsMatchedByRegexOptionsRangeError
{
    ...
    NSString *regex = @"(.*) execution_data: .* (\\w{3}.\\w{3}) .* orderId:(\\d+): clientId:(\\w+), execId:(.*.01), .*, acctNumber:(\\w+).*, side:(\\w+), shares:(\\d+), price:(.*), permId:(\\d+).*";
    XCTAssertTrue([self.candidate isMatchedByRegex:regex options:RKLCaseless range:NSMakeRange(0, [self.candidate length]) error:nil], @"Case-insensitive match has failed!");
    XCTAssertFalse([self.candidate isMatchedByRegex:regex options:RKLNoOptions range:NSMakeRange(0, [self.candidate length]) error:nil], @"Case-sensitive match has failed!");
}
...
- (void)testIsMatchedByRegexRegexOptionsMatchingOptionsRangeError
{
    ...
    NSString *regex = @"(.*) execution_data: .* (\\w{3}.\\w{3}) .* orderId:(\\d+): clientId:(\\w+), execId:(.*.01), .*, acctNumber:(\\w+).*, side:(\\w+), shares:(\\d+), price:(.*), permId:(\\d+).*";
    XCTAssertTrue([self.candidate isMatchedByRegex:regex options:RKLCaseless range:NSMakeRange(0, [self.candidate length]) error:nil], @"Case-insensitive match has failed!");
    XCTAssertFalse([self.candidate isMatchedByRegex:regex options:RKLNoOptions matchingOptions:0 range:NSMakeRange(0, [self.candidate length]) error:&error], @"Case-sensitive match has failed! Error: %@", error);
}
